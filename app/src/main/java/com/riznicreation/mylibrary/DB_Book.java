...
public class DB_Book {
    ...
    private String jasonString(){
        return "{\"Books\" : [\n" +
                ...
                "    \"thumbnailUrl\": \"http://books.google.com/books/content?id=G0Faik1mSp0C&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api\",\n" +
    ...
    private void initData() {
        ...
        try {
            Object[] bookdetails = new Object[7];
            JSONObject jsnobject = new JSONObject(jasonString());
            ...
            for (int i=0;i<jsonArray.length();i++) {
                ...
                {
                    try {
                        bookdetails[0] = jsonArray.getJSONObject(i).get("title").toString();
                    } catch (JSONException e) {
                        bookdetails[0] = "Title not given";
                    }

                    try {
                        bookdetails[1] = jsonArray.getJSONObject(i).get("authors").toString();
                    } catch (JSONException e) {
                        bookdetails[1] = "Author not given";
                    }

                    try {
                        bookdetails[2] = Integer.parseInt( jsonArray.getJSONObject(i).get("pageCount").toString() );
                    } catch (JSONException e) {
                        bookdetails[2] = "0";
                    }

                    try {
                        bookdetails[3] = jsonArray.getJSONObject(i).get("thumbnailUrl").toString();
                    } catch (JSONException e) {
                        bookdetails[3] = "Image not given";
                    }

                    try {
                        bookdetails[4] = jsonArray.getJSONObject(i).get("shortDescription").toString();
                    } catch (JSONException e) {
                        bookdetails[4] = "Short description not given";
                    }

                    try {
                        bookdetails[5] = jsonArray.getJSONObject(i).get("longDescription").toString();
                    } catch (JSONException e) {
                        bookdetails[5] = "";
                    }

                    try {
                        bookdetails[6] = jsonArray.getJSONObject(i).get("buyLink").toString();
                    } catch (JSONException e) {
                        bookdetails[6] = "";
                    }
                }
                ...
                books.add(new Book(
                        (i+1),
                        bookdetails[0].toString(),
                        bookdetails[1].toString(),
                        Integer.parseInt(bookdetails[2].toString()),
                        bookdetails[3].toString(),
                        bookdetails[4].toString(),
                        bookdetails[5].toString(),
                        bookdetails[6].toString()
                ));
            }
